    ...
    try {
      ...
      if (isFed1) {
        ...
        try {
          ...
          dumpMsg = `The (invalid) extracted subgraph has been written in: ${file}.`;
        }
        catch (e2) {
          dumpMsg = `Was not able to print generated subgraph because: ${errorToString(e2, subgraph.name)}`;
        }
        throw new Error(`${msg}.\n\nDetails:\n${errorToString(e, subgraph.name)}\n\n${dumpMsg}`);
      }
    }
