...
export class FederationSpecDefinition extends FeatureDefinition {
  ...
  addElementsToSchema(schema: Schema): GraphQLError[] {
    ...
    errors = errors.concat(fieldSetTypeSpec.checkOrAdd(schema, feature.typeNameInSchema(fieldSetTypeSpec.name)));

    for (const directive of FEDERATION2_SPEC_DIRECTIVES) {
      errors = errors.concat(directive.checkOrAdd(schema, feature.directiveNameInSchema(directive.name)));
    }
    return errors;
  }
