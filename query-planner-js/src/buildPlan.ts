...
class FetchGroup {
  ...
  private constructor(
    ...
    private _selection: MutableSelectionSet,
  ...
  removeInputsFromSelection() {
    ...
    if (inputs) {
      this.cachedCost = undefined;
      this._selection = MutableSelectionSet.of(this.selection.minus(inputs));
    }
  }
  ...
  *     method can be used to ensure that `grandChild` meets those requirement.
  ...
  private finalizeSelection(): { selection: SelectionSet, outputRewrites: FetchDataOutputRewrite[] } {
    ...
    const selectionWithTypenames = addTypenameFieldForAbstractTypes(this.selection);
  ...
  toPlanNode(
    ...
    const { selection, outputRewrites } = this.finalizeSelection();

    const inputs = this._inputs?.get();
    if (inputs) {
      inputs.validate();
    }
...
function addBackTypenameInAttachments(selectionSet: SelectionSet): SelectionSet {
  return selectionSet.lazyMap((s) => {
    ...
    if (typenameAttachment === undefined) {
      ...
      // Note that the value of the "attachement" is the alias or '' if there is no alias
      const alias = typenameAttachment === '' ? undefined : typenameAttachment;
      const typenameField = new Field(s.element.parentType.typenameField()!, {}, new VariableDefinitions(), undefined, alias);
      return [
